...
curl -o  /root/adblocklists/domainlist.txt https://raw.githubusercontent.com/gonzomozg/bash_wg_scrypts/main/domainlist.txt
...
cat /root/adblocklists/domainlist.txt | grep '^127\.0\.0\.1' | awk '{print "local-zone: \""$2"\" always_nxdomain"}' >> /root/adblocklists/unboundadblock_temp.list
